...
rp_module_id="emulationstation-v2.9.6"
...
rp_module_repo="git https://github.com/RetroPie/EmulationStation :_get_branch_emulationstation-v2.9.6"
rp_module_section="main"
rp_module_flags="frontend"

function _get_input_cfg_emulationstation-v2.9.6() {
    echo "$configdir/all/emulationstation/es_input.cfg"
}

function _update_hook_emulationstation-v2.9.6() {
    # make sure the input configuration scripts and launch script are always up to date
    if rp_isInstalled "$md_id"; then
        copy_inputscripts_emulationstation-v2.9.6
        install_launch_emulationstation-v2.9.6
    fi
}

function _sort_systems_emulationstation-v2.9.6() {
    local field="$1"
...
function _add_system_emulationstation-v2.9.6() {
    local fullname="$1"
    local name="$2"
    local path="$3"
    local extension="$4"
    ...
    _sort_systems_emulationstation-v2.9.6 "name"
}

function _del_system_emulationstation-v2.9.6() {
    local fullname="$1"
    local name="$2"
...
function _add_rom_emulationstation-v2.9.6() {
    local system_name="$1"
...
function depends_emulationstation-v2.9.6() {
    local depends=(
...
function _get_branch_emulationstation-v2.9.6() {
    if [[ -z "$branch" ]]; then
...
function sources_emulationstation-v2.9.6() {
    gitPullOrClone
}

function build_emulationstation-v2.9.6() {
...
function init_input_emulationstation-v2.9.6() {
    local es_config="$(_get_input_cfg_emulationstation-v2.9.6)"
...
function copy_inputscripts_emulationstation-v2.9.6() {
    mkdir -p "$md_inst/scripts"

    cp -r "$scriptdir/scriptmodules/$md_type/emulationstation-v2.9.6/"* "$md_inst/scripts/"
    chmod +x "$md_inst/scripts/inputconfiguration.sh"
}

function install_launch_emulationstation-v2.9.6() {
...
_EOF_
    ...
    if isPlatform "x11"; then
        ...
        cp "$scriptdir/scriptmodules/$md_type/emulationstation-v2.9.6/retropie.svg" "/usr/local/share/icons/"
...
function clear_input_emulationstation() {
    rm "$(_get_input_cfg_emulationstation-v2.9.6)"
    init_input_emulationstation-v2.9.6
}

function remove_emulationstation-v2.9.6() {
...
function configure_emulationstation-v2.9.6() {
...
function gui_emulationstation-v2.9.6() {
    local es_swap=0
        ...
        case "$choice" in
            1)
                if dialog --defaultno --yesno "Are you sure you want to reset the Emulation Station controller configuration ? This will wipe all controller configs for ES and it will prompt to reconfigure on next start" 22 76 2>&1 >/dev/tty; then
                    clear_input_emulationstation-v2.9.6
                    printMsgs "dialog" "$(_get_input_cfg_emulationstation-v2.9.6) has been reset to default values."
                fi
                ;;
            2)
                disable="$((disable ^ 1))"
